...
class DCNv2(tf.layers.Layer):
    ...
    def call(self, x):
        ...
        if stride == 2:
            ...
            padding2 = 'same'
        offset_mask = tf.keras.backend.conv2d(temp, self.offset_w, strides=(stride, stride), padding=padding2)
        ...
        out = tf.keras.backend.conv2d(new_x, tw, strides=(1, 1), padding='valid')  # [N, out_H, out_W, out_C]
        return out


if __name__ == '__main__':
    inp = tf.placeholder(tf.float32, [1, 608, 1024, 32])
    pad2 = (3 - 1) // 2
    conv = DCNv2(32, 64, filter_size=3, stride=2, padding=pad2, bias_attr=False)
    out = conv(inp)
    print(out)
